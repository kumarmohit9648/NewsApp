...
import com.knovatik.navadesh.ui.vm.VideoViewModel
...
class VideoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val galleryAdapter = GalleryAdapter { image ->
            // deleteImage(image)
            setResult(GeneratePostActivity.VIDEO, Intent().putExtra("VIDEO", image.contentUri.toString()))
            finish()
        }
 ...
 */
class GalleryViewHolder(view: View, onClick: (Media) -> Unit) :
    ...
    init {
        imageView.setOnClickListener {
            val image = rootView.tag as? Media ?: return@setOnClickListener
            onClick(image)
        }
    }
}